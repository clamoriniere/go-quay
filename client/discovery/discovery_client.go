...
type ClientService interface {
	Discovery(params *DiscoveryParams) (*DiscoveryOK, error)
...
func (a *Client) Discovery(params *DiscoveryParams) (*DiscoveryOK, error) {
	...
	success, ok := result.(*DiscoveryOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for discovery: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
