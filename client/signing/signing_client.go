...
func (a *Client) GetRepoSignatures(params *GetRepoSignaturesParams, authInfo runtime.ClientAuthInfoWriter) (*GetRepoSignaturesOK, error) {
	...
	success, ok := result.(*GetRepoSignaturesOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for getRepoSignatures: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
