...
type AddLabel struct {
	...
	// Required: true
	// Enum: [text/plain application/json null]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["text/plain","application/json","null"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	// AddLabelMediaTypeNull captures enum value "null"
	AddLabelMediaTypeNull string = "null"
)

// prop value enum
func (m *AddLabel) validateMediaTypeEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, addLabelTypeMediaTypePropEnum, true); err != nil {
		return err
	}
	return nil
}
